...
typedef struct ITCAST_ANYBUF_{
    unsigned char *pData;//内存首地址 数据 ----V
    ITCAST_UINT32     dataLen;//内存长度  -----L
    ITCAST_UINT32     unusedBits;  /* for bit string */  
    ITCAST_UINT32     memoryType;
    ITCAST_UINT32     dataType;//数据类型 ----- T
    struct ITCAST_ANYBUF_ *next;    /* for sequence and set *///链表
    struct ITCAST_ANYBUF_ *prev;//双向链表
}ITCAST_ANYBUF;
...
ITCAST_INT
DER_ItAsn1_WriteInteger(ITCAST_UINT32 integer,ITASN1_INTEGER **ppDerInteger);//DER编码整数数据

ITCAST_INT
DER_ItAsn1_ReadInteger(ITASN1_INTEGER *pDerInteger,ITCAST_UINT32 *pInteger);//DER解码整数数据

ITCAST_INT
DER_ItAsn1_WriteBitString(ITASN1_BITSTRING *pBitString,ITASN1_BITSTRING **ppDerBitString);//DER编码BitString类型数据 位串 01构成的串

ITCAST_INT
DER_ItAsn1_ReadBitString(ITASN1_BITSTRING *pDerBitString,ITASN1_BITSTRING **ppBitString);//DER解码BitString类型数据

//DER编码PrintableString类型数据 可打印字符串 字符数组
ITCAST_INT
DER_ItAsn1_WritePrintableString(ITASN1_PRINTABLESTRING *pPrintString,ITASN1_PRINTABLESTRING **ppDerPrintString);

ITCAST_INT
DER_ItAsn1_ReadPrintableString(ITASN1_PRINTABLESTRING *pDerPrintString,ITASN1_PRINTABLESTRING **ppPrintString);//DER解码PrintableString类型数据


ITCAST_INT
DER_ItAsn1_WriteSequence(ITASN1_SEQUENCE *pSequence,ITCAST_ANYBUF **ppDerSequence);//DER编码Sequence类型数据   结构体

ITCAST_INT
DER_ItAsn1_ReadSequence(ITCAST_ANYBUF *pDerSequence,ITASN1_SEQUENCE **ppSequence);//DER解码Sequence类型数据

ITCAST_INT
DER_ItAsn1_WriteNull(ITCAST_ANYBUF ** ppDerNull);//DER编码Null类型数据

ITCAST_INT
DER_ItAsn1_ReadNull(ITCAST_ANYBUF * ppDerNull, ITCAST_UINT8 * pInt);//DER解码Null类型数据

ITCAST_INT 
DER_ITCAST_FreeQueue(ITCAST_ANYBUF *pAnyBuf);

ITCAST_INT
DER_ITCAST_String_To_AnyBuf(ITCAST_ANYBUF **pOriginBuf, unsigned char * strOrigin, int  strOriginLen);//DER编码CharString类型数据

int WriteNullSequence(ITCAST_ANYBUF **pOutData);//编码空结构体
int EncodeChar(char *pData, int dataLen, ITCAST_ANYBUF **outBuf);//编码字符指针
